...
此目录在 Git 源码历史中不直接存放真实定额、清单或 PDF 数据。面向普通用户的完整版 Release 会把已获分发授权的资料安装到应用目录，安装后无需手工配置数据库。
...
只接入你有权处理的资料。如需自行制作发行包，还需要单独确认数据复制和再分发权。数 GB 资料应通过版本化 Release 交付，不应直接写入 Git 历史。
