...
> [!IMPORTANT]
> Git 源码历史和下载包都不携带 PDF、OCR 页、原书路径或开发全文索引。项目的 **完整版 Release** 只提供授权结构化资料库；普通套项不要求另装 PDF，原书仅作为换算、系数和争议项的外部复核资料。API Key 和任何第三方计价软件文件始终不会随软件提供。
...
需要批量部署时，可使用静默参数：`山东定额助手-Setup-0.9.6.exe /VERYSILENT /SUPPRESSMSGBOXES /NORESTART`。

完整版包含山东 2016/2025 定额、2013/2024 清单、清单定额关联和人材机消耗。0.9.6 的结构化运行库约 218 MB，不携带 PDF、OCR 页或原书路径；PDF 页不是确认和导出的强制门禁。资料权利说明与适用边界见 [DATA_NOTICE.md](legal/DATA_NOTICE.md)。
...
- **本地优先**：SQLite 结构化字段和范围索引检索，本地候选不依赖网络或 AI。
