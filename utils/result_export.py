...
_HEADER = ["类型", "编码", "名称", "单位", "专业", "版本", "匹配等级", "页码", "命中原因", "待补条件", "冲突提示"]
...
def proposal_csv(result: dict[str, Any], *, confirmed_only: bool = True) -> list[list[str]]:
    ...
    for proposal in result.get("proposals") or []:
        ...
        if proposal.get("bill_record_id"):
            rows.append([span, "清单", "", str(proposal.get("bill_code") or ""), str(proposal.get("bill_title") or ""), str(proposal.get("bill_unit") or ""), status])
        for quota in proposal.get("quota_lines") or []:
            ...
            rows.append([span, "定额", role, str(quota.get("code") or ""), str(quota.get("title") or ""), str(quota.get("unit") or ""), status])
    return rows


def confirmed_proposal_payload(result: dict[str, Any]) -> dict[str, Any]:
    confirmed = [value for value in result.get("proposals") or [] if value.get("confirmed")]
